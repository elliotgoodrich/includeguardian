...
TEST(GetTotalCostTest, DiamondIncludes) {
  ...
  EXPECT_EQ(get_total_cost::from_graph(graph, {a}), 0b1111 * B);
}
...
TEST(GetTotalCostTest, MultiLevel) {
  ...
  EXPECT_EQ(get_total_cost::from_graph(graph, {a, b}),
            0b1111'1111 * B + graph[d].file_size + graph[f].file_size +
                graph[h].file_size);
}
...
TEST(GetTotalCostTest, LongChain) {
  ...
  EXPECT_EQ(get_total_cost::from_graph(graph, {a}), 0b11'1111'1111 * B);
}
